...
class MQTTResponderThread():
    ...
    def run(self, data):
        ...
        try:
            ...
            while len(self.mids) > 0 and counter < wait_count:
                self.logger.logdbg((f"Client {self.client_id} {len(self.mids)} in flight messages;"
                                    f" on {counter} loop count {self.mids}"))
    ...
    def _on_publish(self, _userdata, mid):
        ...
        self.logger.logdbg(f"Client {self.client_id}: Inflight ({int(time.time())}): {self.mids}")
